...
import { createFeedDiscoverer, type DiscoveredFeed } from "./feed-discovery";
import { createFeedSearcher, type SearchedFeed } from "./feed-search";

export type { DiscoveredFeed, SearchedFeed };

export const discoverFeedsFromUrl = createFeedDiscoverer(async (url) => {
	return await fetchFeed(url);
});

export const searchFeedsByKeyword = createFeedSearcher(
	async (url) => await fetchFeed(url),
);
...
export function parseFeedXml(xmlText: string, feedId: string): ParsedArticle[] {
	...
	const extractImage = (content: string, item: any): string | undefined => {
		...
		const mediaThumbnail =
			item["media:thumbnail"] ?? mediaGroup?.["media:thumbnail"];
		const mediaUrl = mediaContent?.["@_url"] || mediaThumbnail?.["@_url"];
	...
	if (rssItems) {
		...
		for (const item of items) {
			const link =
				normalize(item.link) || normalize(item.guid) || normalize(item["guid"]);
			if (!link) continue;

			const content =
				normalize(item["content:encoded"]) || normalize(item.description) || "";

			const pubDateRaw = normalize(item.pubDate) || normalize(item["dc:date"]);
			const pubDate = pubDateRaw
				? (() => {
						try {
							return new Date(pubDateRaw).toISOString();
						} catch {
							return timestamp;
						}
					})()
			...
			out.push({
				...
				content,
				contentSnippet:
					content.replace(/<[^>]*>/g, "").slice(0, 500) || undefined,
	...
	if (atomEntries) {
		...
		for (const entry of entries) {
			...
			const pubDateRaw =
				normalize(entry.published) || normalize(entry.updated) || "";
			const pubDate = pubDateRaw
				? (() => {
						try {
							return new Date(pubDateRaw).toISOString();
