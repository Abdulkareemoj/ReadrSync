...
export const createReaderStore = (
	...
	loadInitialData: async () => {
		...
		// Load annotations for each highlight and combine
	...
	addHighlight: async (h) => {
		const { highlightAgent } = get();
		const saved = await highlightAgent.addHighlight({
			id: h.id,
		...
		set((s) => ({
			highlights: [...s.highlights, { ...saved, annotations: [] }],
		}));
	},
	removeHighlight: async (id) => {
		const { highlightAgent } = get();
		await highlightAgent.removeHighlight(id);
		set((s) => ({ highlights: s.highlights.filter((h) => h.id !== id) }));
	},
	addAnnotation: async (highlightId, annotation) => {
		const { highlightAgent } = get();
		const saved = await highlightAgent.addAnnotation({
			highlightId,
		...
		set((s) => ({
			highlights: s.highlights.map((h) =>
				h.id === highlightId
					? { ...h, annotations: [...h.annotations, saved] }
					: h,
			),
		}));
	},
	removeAnnotation: async (highlightId, annotationId) => {
		const { highlightAgent } = get();
		await highlightAgent.removeAnnotation(annotationId);
		set((s) => ({
			highlights: s.highlights.map((h) =>
				h.id === highlightId
					? {
