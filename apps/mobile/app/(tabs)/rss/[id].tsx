...
export default function ArticleScreen() {
	...
	console.log(
		"[ArticleScreen] fullContent:",
		(article as any)?.fullContent?.slice(0, 50),
	);
		...
		<SafeAreaView className="flex-1 bg-background">
			...
			>
				<View className="px-5 pt-6 pb-4">
					{feed?.title && (
						<Text className="mb-2 font-semibold text-primary text-xs uppercase tracking-widest">
							{feed.title}
						</Text>
					)}
					<Text className="mb-4 font-bold text-[28px] text-foreground leading-[1.25]">
						{article.title}
					</Text>
					<View className="flex-row items-center gap-3 border-border border-b pb-4">
					...
					{htmlSource.html ? (
						<RenderHtml
							...
							renderersProps={{
								...
								a: {
									onPress: (_e: any, href: string) => Linking.openURL(href),
								},
							}}
							ignoredDomTags={[
								"script",
						...
						<Text className="text-[17px] text-foreground leading-8">
				...
				<View className="mx-5 mt-8 border-border border-t pt-6">
					<Pressable
						onPress={handleOpenSource}
						disabled={!article.link}
